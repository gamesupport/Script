...
SPELL_TYPE = {LINEAR = 0, CIRCULAR = 1, CONE = 3, TARGETTED = 4, SELF = 5, TARGETTED_ALLY = 6}

ORBWALK_MODE = {COMBO = 0, HARASS = 1, CLEAR = 3, LASTHIT = 4, NONE = -1}

CIRCLE_MANAGER = {CIRCLE_2D = 0, CIRCLE_3D = 1, CIRCLE_MINIMAP = 2}

LASTHIT_MODE = { NEVER = 0, SMART = 1, ALWAYS = 2}
...
function _arrangePriorities()
    ...
    if priorityTable~=nil and type(priorityTable) == "table" and enemies > 0 then
        for i, enemy in ipairs(GetEnemyHeroes()) do
            _SetPriority(priorityTable.p1, enemy, priorityOrder[enemies][1])
            _SetPriority(priorityTable.p2, enemy, priorityOrder[enemies][2])
            _SetPriority(priorityTable.p3,  enemy, priorityOrder[enemies][3])
            _SetPriority(priorityTable.p4,  enemy, priorityOrder[enemies][4])
            _SetPriority(priorityTable.p5,  enemy, priorityOrder[enemies][5])
        end
    end
end
                    ...
                    if menu.Enable and condition then
                        ...
                        if source ~= nil and range > 0 then
                            ...
                            elseif mode == CIRCLE_MANAGER.CIRCLE_3D and OnScreen(pos.x, pos.y) then
                                if OnScreen(pos.x, pos.y) and range <= 1800 * 1800 then
...
function _SpellManager:InitMenu()
    if _G.SpellManagerMenu == nil then
        _G.SpellManagerMenu = scriptConfig("Spell Manager", "SpellManager".."19052015"..myHero.charName)
    end
end
    ...
    if tab ~= nil then
        ...
        if self:IsSkillShot() then
            ...
            if self.Menu ~= nil then
                ...
                self.Menu:addParam("info2", "60 % ~ High Accuracy", SCRIPT_PARAM_INFO, "")
...
function _Spell:AddDraw()
    self:AddToMenu()
    if self.Menu~=nil then
        _Circle({Menu = self.Menu, Name = self.Name.."Drawing", Text = "Drawing Settings", Source = function() return self.Source end, Range = function() return self.Range end, Condition = function() return self:IsReady() end})
    end
    return self
end

--[[
function _Spell:SetAdjustableRange()
end

function _Spell:AddLastHit(int)
    self:AddToMenu()
    if self.Menu ~= nil then
        local default = int ~= nil and type(int) == "number" and int or 2
        self.Menu:addSubMenu("Last Hit Settings", "LastHit")
            self.Menu.LastHit:addParam("Mode", "Mode", SCRIPT_PARAM_LIST, default, {"Never", "Smart", "Always"})
            self.Menu.LastHit:addParam("Delay", "Delay (in ms)", SCRIPT_PARAM_SLICE, 0, -150, 150)
    end
    self.LastHitMode = int ~= nil and type(int) == "number" and int or LASTHIT_MODE.SMART
    return self
end
]]

function _Spell:SetRangeFunction(rngFunc)
...
function _Spell:SetTypeFunction(f)
...
function _Spell:SetSourceFunction(srcFunc)
...
function _Spell:Cast(target)
    if self:IsReady() and target ~= nil and target.valid then
        if GetDistanceSqr(self.Source, target) <= self.Range * self.Range then
            ...
            elseif self:IsTargettedAlly() then
                if IsValidTarget(target, self.Range, false) then
                    CastSpell(self.Slot, target)
                end
            elseif self:IsSelf() then
                if IsValidTarget(target) then
...
function _Spell:GetPrediction(target, t)
    if IsValidTarget(target) then
        if self:IsSkillShot() then
            ...
            local tab = {Delay = self.Delay, Width = self.Width, Speed = self.Speed, Range = self.Range, Source = source, Type = self.Type, Collision = self.Collision, Aoe = self.Aoe, TypeOfPrediction = pred, Accuracy = accuracy}
            return Prediction:GetPrediction(target, tab)
        elseif self:IsSelf() then
            return Prediction:GetPredictedPos(target, self.Delay)
        end
    end
...
function _Spell:IsTargettedAlly()
    return self.Type == SPELL_TYPE.TARGETTED_ALLY
end
...
function _Spell:ValidTarget(target)
    return IsValidTarget(target) and GetDistanceSqr(self.Source, target) <= self.Range * self.Range
end
...
function _Spell:Mana()
    return myHero:GetSpellData(self.Slot).mana
end
...
function _Spell:LaneClear(tab)
    if self.EnemyMinions == nil then
        self.EnemyMinions = minionManager(MINION_ENEMY, self.Range, self.Source, MINION_SORT_MAXHEALTH_DEC)
    end
...
function _Spell:JungleClear(tab)
    if self.JungleMinions == nil then
        self.JungleMinions = minionManager(MINION_JUNGLE, self.Range, self.Source, MINION_SORT_MAXHEALTH_DEC)
    end
                ...
                if self:IsReady() and self:ValidTarget(object) and not object.dead and object.health > 0 then
                    ...
                    if mode == LASTHIT_MODE.SMART then
                        if not OrbwalkManager:CanAttack() then
                            if object ~= OrbwalkManager.AA.LastTarget and not OrbwalkManager:IsAttacking() then
                                CanCalculate = true
                            end
                        else
                            if not OrbwalkManager:InRange(object, 80) then
                                CanCalculate = true
                            end
                        end
                    ...
                    if CanCalculate then
                        ...
                        if predHealth == object.health and mode == LASTHIT_MODE.SMART then return end 
...
function _Prediction:__init()
    ...
    if VIP_USER and FileExist(LIB_PATH.."DivinePred.lua") and FileExist(LIB_PATH.."DivinePred.luac") then require "DivinePred" self.DP = DivinePred() table.insert(self.PredictionList, "DivinePred") end
    if FileExist(LIB_PATH.."HPrediction.lua") then require "HPrediction" self.HP = HPrediction() table.insert(self.PredictionList, "HPrediction") end
    self.LastRequest = 0
...
function _Prediction:GetPrediction(target, sp)
    ...
    if IsValidTarget(target) and self:ValidRequest(TypeOfPrediction) then
        ...
        elseif TypeOfPrediction == "DivinePred" then
            ...
            if skillshotType == SPELL_TYPE.LINEAR then
                mySkill = LineSS(speed, range, width, delay * 1000, col)
            elseif skillshotType == SPELL_TYPE.CIRCULAR then
                mySkill = CircleSS(speed, range, width, delay * 1000, col)
            elseif skillshotType == SPELL_TYPE.CONE then
                mySkill = ConeSS(speed, range, width, delay * 1000, col)
            end
            local state, pos, perc = self.DP:predict(DPTarget(target), mySkill, self:AccuracyToHitChance(TypeOfPrediction, accuracy), Vector(source))
...
function _Prediction:GetPredictedPos(unit, delay, speed, from, collision)
    local CastPosition, HitChance, Position = self.VP:GetPredictedPos(unit, delay, speed, from, collision)
    local WillHit = false
    if HitChance > 0 then
        WillHit = true
    else
...
function _OrbwalkManager:__init()
    ...
    if _G.OrbwalkManagerMenu == nil then
        _G.OrbwalkManagerMenu = scriptConfig("Orbwalk Manager", "OrbwalkManager".."24052015"..myHero.charName)
    end
    ...
    AddCreateObjCallback(
        function(obj)
            if self.AA.Object == nil and obj.name:lower() == "missile" and not self:CanMove() and GetDistanceSqr(Vector(myHero.x, 0, myHero.z), Vector(obj.x, 0, obj.z)) < 150 * 150 then
                self.AA.Object = obj
            end
        end
    )
    ...
    AddDeleteObjCallback(
        function(obj)
            if obj and self.AA.Object ~= nil and obj.name == self.AA.Object.name and GetDistanceSqr(obj, self.AA.Object) < 100 * 100 then
                self:ResetMove()
            end
        end
    )
    ...
    if unit and unit.isMe and spell and spell.name then
        ...
        elseif spell.name:lower():find("riventricleave") or spell.name:lower():find("tiamat") or spell.name:lower():find("hydra") then
            ...
            DelayAction(
                ...
                ,spell.windUpTime - 2 * GetLatency()/2000)
        end
    end
end
...
function _OrbwalkManager:TakeControl()
    _G.OrbwalkManagerMenu:addParam("ExtraWindUp","Extra WindUpTime", SCRIPT_PARAM_SLICE, 20, -40, 160, 1)
...
function _OrbwalkManager:OrbLoad()
    if self:GetOrbwalkSelected() ~= nil then
        ...
        if self:GetOrbwalkSelected() == "SxOrbWalk" then
            require 'SxOrbWalk'
            self.OrbLoaded = self:GetOrbwalkSelected()
