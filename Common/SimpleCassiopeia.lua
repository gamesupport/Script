...
function OnLoad()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 850, Width = 75, Delay = 0.6, Speed = math.huge, Collision = false, Aoe = true, Type = SPELL_TYPE.CIRCULAR}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 850, Width = 90, Delay = 0.5, Speed = 2500, Collision = false, Aoe = true, Type = SPELL_TYPE.CIRCULAR}):AddDraw()
    E = _Spell({Slot = _E, DamageName = "E", Range = 700, Delay = 0, Speed = 1900, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    R = _Spell({Slot = _R, DamageName = "R", Range = 800, Delay = 0.3, Width = 80, Speed = math.huge, Collision = false, Aoe = true, Type = SPELL_TYPE.CONE}):AddDraw()

    local igniteslot = FindSummonerSlot("summonerdot")
    if igniteslot ~= nil then
        Ignite = _Spell({Slot = igniteslot, DamageName = "IGNITE", Range = 600, Type = SPELL_TYPE.TARGETTED})
    end
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addParam("useR2", "Use R If Enemies >=", SCRIPT_PARAM_SLICE, 3, 0, 5, 0)
...
function KillSteal()
    for idx, enemy in ipairs(GetEnemyHeroes()) do
        if ValidTarget(enemy, TS.range) and enemy.health > 0 and enemy.health/enemy.maxHealth <= 0.3 then
            ...
            if Menu.KillSteal.useIgnite and Ignite:Damage(enemy) >= enemy.health and not enemy.dead then Ignite:Cast(enemy) end
        end
    end
end
